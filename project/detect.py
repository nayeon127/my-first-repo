    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if results.multi_face_landmarks:                       
            for face_landmarks in (results.multi_face_landmarks):                
                ...
                # # time
                # gaze_model.timeCalculation(begin)
        ...
        gridLine(gaze_model.eyes_df, im0s, range_w, eye_list[0], eye_list[2], x, y)            
            # # put text
            # putText(im0s, dir_total, ear, x, y)
        # Gaze line
        gazeLine(im0s, dir_total, ear, font, x, y)
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                im0s, top_iou_obj, total_fps_cnt, cell_phone_xyxy = object_select_model.IOUandTime(im0s, names, colors, det, box1, x, y)
                # top_iou_obj, total_fps_cnt, cell_phone_xyxy = object_select_model.IOUandTime(im0s, out, names, colors, det, box1, x, y)

                # cell_phone_xyxy, top_iou_obj, top_iou_for10fps, fps_cnt, total_fps_cnt, top_iou = IOUandTime(im0s, names, colors, det, box1_x1, box1_x2, box1_y1, box1_y2, top_iou_for10fps, top_iou_obj, fps_cnt, total_fps_cnt, top_iou, x, y)
                # gaze point line
                if ear != 'UP':
                    gazePointLine(im0s, face_landmarks, ear, gaze_line_x, gaze_line_y, x, y, cell_phone_xyxy, top_iou_obj)                
                out.write(im0s)
                # cv2_imshow(im0s)  
        ...
        im0s = noFaceMesh(im0s, out, total_fps_cnt, x, y)
